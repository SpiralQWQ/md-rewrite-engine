...
- 每次重写必须过**机械门禁**（数字硬判定 ≥90% / 6 件套齐全 / 费曼示范 / **丰富度膨胀率** / GLM 评分达标）——任一不达标即打回，绝不"信任通过"；
...
- **fail-closed 门禁**：机械对账（数字硬判定 ≥90%）/ 6 件套完整 / 费曼示范 / **丰富度膨胀率（v0.4.5）** / GLM 质检评分任一不达标 → 不能「通过」。
- **机器门禁一键检查**：`scripts/gate_check.py`——本体重排完必跑，一次拿到机械对账/6件套/费曼/**丰富度膨胀率**/评分硬指标，不达标强制打回（≤4 次 → 挂起），**质量靠门禁不靠自觉**。
- **丰富度门禁**（v0.4.5）：膨胀率 = 笔记内容量/源内容量（三区间：≥80% 直过 / [30%,80%) 聚合豁免三条件 / <30% 绝对红线）——专治「把完整推导压缩成提词卡」（机械对账 100% 也拦得住）。
...
```bash
python -m pytest tests/ -q          # 单元测试 199
